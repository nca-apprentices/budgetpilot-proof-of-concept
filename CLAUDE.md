...
- ⏳ Offen: lokale Datenbank, Preislogik (toppreise.ch), PDF-Export, Golden Set, Tests auf echten Geräten (aktuell nur Simulator), Umstieg auf multimodales Modell für Kamera-/Beleg-Scan
...
| PDF-Erstellung | Noch offen: `react-native-html-to-pdf` oder `pdf-lib` |
...
5. **Zusammenfassung & Export**
   - KI-Textzusammenfassung (kurz + detailliert)
   - PDF-Export (1–2 Seiten)
6. **Offline-Nachweis**
...
- PDF exportiert ohne Layoutfehler
...
4. Validierung & Post-Processing (Beträge, Intervalle, Kategorien) → 5. Persistenz lokal
6. Budget Engine → 7. Summary Generator (LLM, templated prompts) → 8. PDF-Export
...
- **LineItem** (`budget.ts`): `id`, `description`, `amount`, `currency`, `cadence` (monthly/one_time), `category`, `source` (free_text/manual/toppreise — nur `free_text` aktuell angebunden), `confidence`, `notes`
...
- ⏳ Zusammenfassung (kurz + detailliert) + PDF-Export
