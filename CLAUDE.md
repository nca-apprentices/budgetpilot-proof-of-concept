...
- ✅ Foto-Extraktion technisch funktionsfähig, aber **inhaltliche Genauigkeit ungenügend** (siehe Risiken): Entwurf-Screen zeigt das hochgeladene Beleg-Foto direkt über den Feldern an (visueller Abgleich), Sicherheitsnetz (needs_input/Low-Confidence-Rahmen) funktioniert — aber die extrahierten Werte selbst waren bei 2/2 getesteten echten Kassenzetteln falsch (siehe Lessons Learned)
...
- **Foto-Extraktion (Stichprobe, nicht repräsentativ):** 0/2 echte Kassenzettel korrekt extrahiert (Betrag und Beschreibung jeweils falsch) — deutet darauf hin, dass die 85%/80%-Ziele oben für den Foto-Pfad mit dem aktuellen Modell/Prompt aktuell nicht erreichbar sind. Vor einer belastbaren Aussage braucht es ein größeres Testset echter Belege (Golden Set).
...
- **Neu, bestätigt (Foto-Extraktion halluziniert):** Bei 2 von 2 getesteten *echten* fotografierten Kassenzetteln (Coop Restaurant, CHF 9.95 und CHF 37.10) hat Gemma 4 E2B-it den Betrag und die Beschreibung komplett falsch extrahiert (u. a. "Kauf von einem Kaktus" für einen Kaffee/Saft-Beleg) — einmal davon mit fälschlich hoher Confidence (95%), was das Confidence-basierte Sicherheitsnetz aushebelt. Die technische Bildübertragung ist nachweislich korrekt (Byte-Länge geprüft, reales JPEG kommt an), es handelt sich also um eine echte Modell-Grenze bei der Fein-Text-/OCR-Lesung aus Fotos, nicht um einen Integrationsbug. Mitigation/offen: mehr echte Belege für ein belastbares Bild ansammeln, ggf. Nutzer-Hinweis "Foto-Extraktion weniger zuverlässig als Freitext" einblenden, Confidence-Wert beim Foto-Pfad nicht blind vertrauen.
